...
export default async (context, inject) => {
  ...
  let api = {}
  try {
    api = await Prismic.api('https://testfemi.cdn.prismic.io/api/v2', Object.assign({}, options,  {"routes":[{"type":"page","path":"/:uid"}]}))
  } catch (error) {
    console.error(error)
  ...
  let prismic = new Vue({
    computed: {
      ...
      apiEndpoint() {
        return 'https://testfemi.cdn.prismic.io/api/v2'
      },
      predicate() {
  ...
  if (process.client) {
    window.onNuxtReady && window.onNuxtReady((app) => {
      ...
      script.src = '//static.cdn.prismic.io/prismic.min.js?repo=testfemi&new=true'
